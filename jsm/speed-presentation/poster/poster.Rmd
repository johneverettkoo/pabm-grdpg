...
**Theorem**: $\max_i \| \hat{x}_i - Q_n x_i \| = O_P \Big( \frac{\log^c n}{n^{1/2}} \Big)$ for some $Q_n \in \mathbb{O}(p, q)$.
...
```{r, fig.height = 2.2, fig.width = 6, out.width = '100%'}
...
dcbm.graph.plot <- ggraph(A, layout = 'stress') + 
  ...
  geom_node_point(aes(colour = factor(z)),
                  show.legend = FALSE) + 
  labs(colour = NULL, 
       title = 'Degree Corrected Block Model') + 
  # theme_graph() + 
  theme_void() + 
  scale_colour_brewer(palette = 'Set1') 
...
pabm.graph.plot <- ggraph(A, layout = 'stress') + 
  ...
  geom_node_point(aes(colour = factor(z)),
                  show.legend = FALSE) + 
  labs(colour = NULL, 
       title = 'Popularity Adjusted Block Model') + 
  # theme_graph() + 
  theme_void() + 
  scale_colour_brewer(palette = 'Set1') 
...
1. Decompose $A \approx V_{p,q} \Lambda_{p,q} V_{p,q}^\top$, $p = \frac{K (K+1)}{2}$ and $q = \frac{K (K-1)}{2}$.
2. Compute $B = |n V_{p,q} V_{p,q}^\top|$.
3. Construct graph $G'$ from $B$.
4. Partition $G'$ into $K$ disconnected subgraphs.

**Theorem** (KTT): 
Let $B$ be the affinity matrix from OSC. 
Then $\forall$ pairs $(i, j)$ belonging to different communities 
and sparsity factor satisfying $n \rho_n = \omega\big(\log^{4c} n\big)$,  
$\max_{i, j} B_{ij} = O_P \Big( \frac{\log^c n}{\sqrt{n \rho_n}} \Big)$.

## Simulation Study

```{r, fig.height = 2, fig.width = 5, out.width = '100%'}
...
Exploiting the geometry of PABMs (and other block models) results in consistent community detection algorithms.