...
1. Eigendecompose $A \approx V_{p,q} \Lambda_{p,q} V_{p,q}^\top$ with $p = \frac{K (K+1)}{2}$ and $q = \frac{K (K-1)}{2}$.
...
Then $\forall$ $(i, j)$ belonging to different communities and sparsity factor satisfying $n \rho_n = \omega\big(\log^{4c} n\big),$ $\max_{i, j} B_{ij} = O_P \Big( \frac{\log^c n}{\sqrt{n \rho_n}} \Big)$.
