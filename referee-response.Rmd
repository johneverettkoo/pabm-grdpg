...
\textcolor{blue}{
We thank the referee for the feedback. The referee's comments are
answered below. 
In particular, we made considerable effort to address the referee's concerns about the simulation studies. 
}
...
the accuracy of OSC is always comparable and/or better than the remaining algorithms for all levels of the sparsity
considered.
}
...
\textcolor{blue}{
For the simulation studies, we only considered $K \leq 4$ as larger
$K$ will, in general, require larger values of $n$ to achieve
comparable estimation accuracy as lower values of $K$. The running
time for a simulation studies when $n$ is large then becomes quite
prohibitive, and this is due mainly to the running-time of the SSC-A
algorithm (see the discussion about computationaly
complexity); the running time of the SSC-ASE and OSC is still
reasonable when we increases $n$. We believe that our current simulation
studies (including the new section 4.3) is sufficiently
comprehensive to illustrate our theoretical results results; indeed
our theoretical results are large-sample results and is valid for any
choice of $K$. The only reason for needing large
$n$ when $K$ increases is that the rank of $P$ is equal to $K^2$ and
hence, for a given $n$, estimation of $P$ when $K$ is say $4$ is
considerably simpler than estimation of $P$ when $K$ is say $8$. 
}  
\textcolor{blue}{
For the computational complexity of the algorithms, 
...
In practice the choice of parameter $K$ does not affect runtimes too much in our simulation studies, 
...
detection property (SDP). However, as other researchers had pointed
out that, even in the context of sparse subspace clustering where the points lie
exactly on a union of $K$ subspaces, the fact that SDP is satisfied
...
$K$ connected component (see e.g., \citep{liu_ssc,sdp_sufficiency,}).
Indeed, we found in our simulations that
