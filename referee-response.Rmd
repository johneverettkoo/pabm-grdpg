...
in general, require larger values of $n$ to achieve comparable estimation accuracy as lower values of $K$ and SSC-A 
(where SSC is applied to the rows of the adjacency matrix) takes a very long time to run when $n$ is large  (see the discussion about computational complexity below). 
Our theoretical results are limit results and work for any choice of $K$ as long as $n$ is large. The reason for needing large $n$ when $K$ increases is that the rank of $P$ is equal to $K^2$ and hence, for a given $n$, estimation of $P$ when $K$ is say $4$ is considerably simpler than estimation of $P$ when $K$ is say $8$. 
We have performed some cursory numerical experiments and found that OSC and SSC-ASE for $K = 8$ and $n = 4096$ behave similarly to $K \leq 4$ and $n = 1024$.
}  
\textcolor{blue}{
