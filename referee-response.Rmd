...
are i.i.d $\mathrm{Beta}(1,2)$ random variables and $\lambda_{k\ell}$ is a
...
\textcolor{blue}{
Sparsity was not considered in our analyses and algorithms. (Fix this)
}
...
space is always unique). Similarly, SSC-ASE also circumvents this non-identifiability
by only using the rows of $n^{1/2} \hat{V}$ as two rows $\hat{v}_i$ and
$\hat{v}_j$ of $\hat{V}$ belonging to different subspaces, i.e.,
different communities, will also
have $n \hat{v}_i^{\top} \hat{v}_j \approx 0$. 
}  
\textcolor{blue}{
That said, looking back at the latent configuration in Theorem 1, we
can see that a $K$-dimensional (not $K^2$-dimensional) embedding is
...
Nevertheless we have found via numerical experiments that community
detection is often possible using only the eigenvectors corresponding
