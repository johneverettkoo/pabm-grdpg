...
and the projection matrix $\Pi$, which is an ASE of $P$. $Z = X Q - Y$ for 
...
Then $Z^{(n)} \to 0$, $\delta^{(n)} \to 0$, and 
$r(X^{(n, l)} Q^{(n, l)}) \to r(Y^{(n, l)})$. Here we assume 
$r(Y^{(n, l)}) > 0$ $\forall n > 3$ and $l \leq K$. 
...
**Theorem 5**.
The subspace detection property holds for $X^{(n)} Q^{(n)}$ as $n \to \infty$. 
...
**Theorem 7**.
***TODO*** Find the probability for which the subspace detection property holds 
for $X^{(n)}$ or $X^{(n)} Q^{(n)}$.